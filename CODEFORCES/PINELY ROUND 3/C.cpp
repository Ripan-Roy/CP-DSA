...
void helper()
{
    ...
    ll ans = 0;
    vll weights;
    vector<pair<ll ,char>> pairs;
    FOR(i, 0, n)
    {
        pairs.push_back({lArr[i], 'l'});
        pairs.push_back({rArr[i], 'r'});
    }
    vll temp;
    sort(ALL(pairs));
    sort(ALL(cArr), greater<ll>());
    for (auto it : pairs)
    {
        if (it.S == 'r')
        {
            weights.push_back((it.F - temp.back()));
            temp.pop_back();
        }
        else
        {
